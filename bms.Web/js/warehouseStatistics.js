...
$(document).ready(function () {
    ...
    $("#exportAll").click(function () {
        if (!$("#table td:visible").length) {
            ...
            var time = $("#time").val();
            window.location.href = "warehouseStatistics.aspx?op=exportAll&&&&groupbyType=" + groupbyType + "&&supplier=" + supplier + "&&regionName=" + regionName + "&&time=" + time;
        }
    })
    //导出报表明细
    $("#exportDe").click(function () {
        if (!$("#table td:visible").length) {
            ...
            var time = $("#time").val();
            window.location.href = "warehouseStatistics.aspx?op=exportDe&&groupbyType=" + groupbyType + "&&supplier=" + supplier + "&&regionName=" + regionName + "&&time=" + time;
        }
    })
    //清空时间
...
function logout() {
    swal({
        ...
        $.ajax({
            type: 'get',
            url: 'warehouseStatistics.aspx?op=logout',
            datatype: 'text',
            data: {},
            success: function (data) {
...
function AddTitle() {
    ...
    LODOP.PRINT_INIT("出库统计");
    LODOP.SET_PRINT_PAGESIZE(3, 1500, 100, "");
    LODOP.ADD_PRINT_TEXT(6, 280, 130, 30, "出库统计");
