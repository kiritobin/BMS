...
namespace bms.Dao
{
    public class WarehousingDao
    {
        ...
        public DataTable exportDel(string groupbyType, string strWhere, int type)
        {
            ...
            {
                cmdText = "select ISBN,bookNum as 书号,bookName as 书名,uPrice as 定价,sum(number) as 数量,sum(totalPrice) as 码洋,sum(realPrice) as 实洋,remarks as 销售折扣,time as 制单日期,userName as 制单人, supplier as 供应商, regionName as " + regionName + " from v_monomer where type=" + type + " and " + strWhere + ",booknum,userName order by convert(" + groupbyType + " using gbk) collate gbk_chinese_ci";
            }
            else
            {
