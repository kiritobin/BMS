...
namespace bms.Dao
{
    public class SaleMonomerDao
    {
        ...
        {
            string cmdText = "select bookNum,number from v_persalemonomer where deleteState=0 and saleTaskId=@saleTaskId and saleHeadId=@saleHeadId";
            string[] param = { "@saleTaskId", "@saleHeadId" };
            object[] values = { saleTaskId, saleHeadId };
            float sltemp = 0;
            int zl = 0;
        ...
        {
            string cmdText = "select singleHeadId,ISBN,number,uPrice,discount,totalPrice,realPrice,shelvesName,bookName from V_Monomer where singleHeadId=@singleHeadId and deleteState=0 order by time desc";
        ...
        /// <returns>数据集</returns>
        public int wechatPerDel(string condition, int state)
        {
            string cmdtext;
            int rows;
            ///3为预采（数据采集）
        ...
        {
            String cmdText = "select bookNum as 书号,bookName as 书名,ISBN as ISBN,unitPrice as 单价,sum(number) as 数量 ,sum(totalPrice) as 码洋,supplier as 出版社,author as 销售折扣 from v_salemonomer where saleHeadId='" + strWhere + "' group by bookNum,bookName,ISBN,unitPrice  HAVING 数量!=0";
            DataSet ds = db.FillDataSet(cmdText, null, null);
            DataTable dt = null;
            if (ds != null && ds.Tables[0].Rows.Count > 0)
            {
        ...
        public DataTable exportDel(string groupbyType, string strWhere)
        {
            ...
            {
                cmdText = "select " + groupbyType + " as " + name + ", ISBN,bookNum as 书号,bookName as 书名,price as 定价,sum(number) as 数量,sum(totalPrice) as 码洋,sum(realPrice) as 实洋,remarks as 销售折扣,DATE_FORMAT(dateTime,'%Y-%m-%d %H:%i:%s') as 采集日期,userName as 采集人用户名, state as 采集状态,supplier as 供应商,dentification as 备注,remarksOne as 备注1,remarksTwo as 备注2,remarksThree as 备注3 from v_salemonomer where " + strWhere + ",booknum,userName HAVING 数量!=0 order by convert(" + groupbyType + " using gbk) collate gbk_chinese_ci";
            }
            else
            {
