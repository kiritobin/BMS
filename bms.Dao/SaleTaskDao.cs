...
namespace bms.Dao
{
    public class SaleTaskDao
    {
        ...
        {
            String  cmdText = "select bookNum as 书号,bookName as 书名,ISBN as ISBN,unitPrice as 单价,sum(number) as 数量 ,sum(totalPrice) as 码洋,supplier as 出版社,author as 销售折扣,state as 单据状态 from v_allsalemonomer where saleTaskId='" + strWhere + "'  group by bookNum,bookName,ISBN,unitPrice having 数量!=0";
