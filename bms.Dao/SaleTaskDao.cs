...
namespace bms.Dao
{
    public class SaleTaskDao
    {
        ...
        public DataSet getAllprice()
        {
            ...
            //DataSet ds = db.FillDataSet(cmdText, param, values);
            //return ds;
            string sql = "select count(bookNum) as allKinds,sum(allNum) as allNum,sum(allPrice) as allPrice,sum(allRealPrice) as allRealPrice from (select sm.ISBN,sm.bookNum,book.bookName,sm.unitPrice,sum(sm.number) as allNum,sum(sm.realPrice) as allRealPrice,sum(sm.totalPrice) as allPrice,head.state,sm.dateTime,us.userID,us.userName,ct.customerID,ct.customerName,rg.regionName from t_salemonomer as sm,t_bookbasicdata as book,t_user as us,t_customer as ct,t_salehead as head,t_saletask as task,t_region as rg where (head.state=1 or head.state=2) and sm.saleTaskId = task.saleTaskId and task.userId = us.userID and sm.bookNum = book.bookNum AND sm.ISBN = book.ISBN and task.customerId = ct.customerID and us.regionId = rg.regionId and head.userId = us.userID AND head.regionId = rg.regionId AND head.saleTaskId = task.saleTaskId AND sm.saleHeadId = head.saleHeadId GROUP BY sm.bookNum) as statics";
        ...
        public DataSet getAllpriceRegion(int regionId)
        {
            ...
            //DataSet ds = db.FillDataSet(cmdText, param, values);
            //return ds;
            string cmdText = "select count(bookNum) as allKinds,sum(allNum) as allNum,sum(allPrice) as allPrice,sum(allRealPrice) as allRealPrice from (select sm.ISBN,sm.bookNum,book.bookName,sm.unitPrice,sum(sm.number) as allNum,sum(sm.realPrice) as allRealPrice,sum(sm.totalPrice) as allPrice,head.state,sm.dateTime,us.userID,us.userName,ct.customerID,ct.customerName,rg.regionName from t_salemonomer as sm,t_bookbasicdata as book,t_user as us,t_customer as ct,t_salehead as head,t_saletask as task,t_region as rg where (head.state=1 or head.state=2) and sm.saleTaskId = task.saleTaskId and task.userId = us.userID and sm.bookNum = book.bookNum AND sm.ISBN = book.ISBN and task.customerId = ct.customerID and us.regionId = rg.regionId and head.userId = us.userID AND head.regionId = rg.regionId AND head.saleTaskId = task.saleTaskId AND sm.saleHeadId = head.saleHeadId and us.regionId=@regionId GROUP BY sm.bookNum) as statics";
